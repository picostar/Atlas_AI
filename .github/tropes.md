...
### Invented or Misapplied Terms

Coining words that do not exist ("vendorable") or stretching a real technical term past its actual meaning to sound precise. If a term has an established definition, use it only for that meaning; if no accepted term fits, describe the thing plainly instead of borrowing jargon that is close but wrong. Made-up or misapplied terminology reads as authoritative while being inaccurate, which is worse than plain description. When challenged on a term, cite a primary source for the exact usage or drop it -- do not cite a source that uses the word in a different sense.

**Avoid patterns like:**
- "vendorable" (not a word)
- calling a hand-copied source include "vendoring" (that term means a package manager copying declared dependencies into a local directory)
- reaching for a near-miss term to sound precise instead of stating plainly what happens
...
Compulsive overuse of em dashes for dramatic pauses, parenthetical asides and pivot points. A human writer might use 2-3 per piece (and naturally); AI will use 20+. The ASCII substitute, the double hyphen ` -- `, is the same tic in disguise and does not fix it. Do not reach for either. Use regular English sentence structure, syntax, and grammar: give the aside its own sentence, or set it off with a comma, a colon, or parentheses.
...
**Write instead:**
- "The problem is systemic, and that is the part nobody talks about."
- "The tinkerer spirit did not die of natural causes. It was bought out."
- "Not recklessly and not completely, but enough to matter."
